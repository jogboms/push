...
Class Application extends Collection
   ...
   */
    ...
    {
      $m = $this->environment['APP_MODEL_DIR'].'\\'.$model;
      return new $m($this->getDB());
    }
    final public function controller($controller)
    {
      $c = $this->environment['APP_CONTROLLER_DIR'].'\\'.$controller;
      return new $c($this);
    }
  ...
  final private function boot($params)
  {
    ...
    defined('WEB_DOMAIN') or define('WEB_DOMAIN', $this->request->getDomain());
    defined('WEB_ROOT') or define('WEB_ROOT', empty($this->request->getBasePath()) ? '/': $this->request->getBasePath());
    ...
    defined('APP') or define('APP', RootPath($env['APP_DIR']));
    defined('STORAGE') or define('STORAGE', RootPath($env['APP_STORAGE_DIR']));
    defined('LIBRARY') or define('LIBRARY', RootPath($env['APP_LIBRARY_DIR']));
    defined('INCLUDES') or define('INCLUDES', RootPath($env['APP_INCLUDES_DIR']));
    ...
    // Set Debugger Mode
    $this->debug_mode($env['APP_USE_DEBUGGER']);

    // Boot Uploads
    defined('UPLOADS') or define('UPLOADS', RootPath($env['APP_UPLOADS_DIR'], true));
    defined('UPLOADS_URI') or define('UPLOADS_URI', ROOT_URI.'/'.trim($env['APP_UPLOADS_DIR'], '/'));

    // Boot View and Templating
    defined('ASSET_URI') or define('ASSET_URI', ROOT_URI.'/'.trim($env['APP_ASSET_DIR'], '/'));
    defined('ASSET_PATH') or define('ASSET_PATH', RootPath($env['APP_ASSET_DIR'], true));
    defined('TEMPLATE_PATH') or define('TEMPLATE_PATH', RootPath($env['APP_VIEW_DIR']));
    ...
    if($env['APP_DEBUG']){
      $this->router->any('/__environment__', function($req, $res) use($env){
  ...
  final private function prepareRoute(Route $R)
  {
    ...
    if(is_int($R['controller']) || is_string($R['controller'])){
      ...
      $ctrl = $this->environment['APP_CONTROLLER_DIR'].'\\';
    ...
    try {
      ...
      $this->router->any('/', function($req, $res) use($env){
          $res->send('
              ...
              <p>Get Started!<br /><a href="__environment__">Environment Variables!</a>&nbsp;/&nbsp;<a href="world">Hello World!</a></p><br /><br />'.$env->get('LICENSE').'</body></html>'
              );
        });
